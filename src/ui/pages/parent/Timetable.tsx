...
import { BlocParentTimetable, QuizClassroomLite, QuizSubjectLite, QuizLessonLite } from "../../bloc/BlocParentTimetable";
...
// khong co gio giac (chi thu tu mon trong ngay), moi mon gan dung 1 Lesson co san.
//
// Chon Lop o dau trang (neu Phu huynh co nhieu hon 1 Lop) -> 7 the ngay Thu Hai..Chu Nhat, moi the
// liet ke cac Lesson da xep theo thu tu + nut Sua mo Dialog thay TOAN BO danh sach ngay do (chon
// Mon hoc -> chon Bai hoc -> Them vao danh sach dang soan, co the xoa/doi thu tu truoc khi Luu).
export default function ParentTimetable() {
    ...
    const [pickLessonId, setPickLessonId] = useState<number | ''>('');
    ...
    const addLesson = (subjects: QuizSubjectLite[], lessonsBySubject: Record<number, QuizLessonLite[]>) => {
        if (pickSubjectId === '' || pickLessonId === '') return;
        const subject = subjects.find((s) => s.id === pickSubjectId);
        const lesson = (lessonsBySubject[pickSubjectId as number] ?? []).find((l) => l.id === pickLessonId);
        if (!subject || !lesson) return;
        bloc.addDraftLesson({ lessonId: lesson.id, lessonName: lesson.name, subjectName: subject.name });
        setPickLessonId('');
    };
                            ...
                            return (
                                <UIStream
                                    ...
                                    builder={(subjectsSnap) => {
                                        ...
                                        return (
                                            <UIStream
                                                initialData={bloc.getField('lessonsBySubject') ?? {}}
                                                stream={bloc.getStream('lessonsBySubject')}
                                                builder={(lessonsSnap) => {
                                                    const lessonsBySubject: Record<number, QuizLessonLite[]> = lessonsSnap.data ?? {};
                                                    return (
                                                        <Stack spacing={2}>
                                                            <Card sx={{ p: { xs: 2, sm: 3 } }}>
                                                                                                ...
                                                                                                <Stack spacing={0.5}>
                                                                                                    {dayEntries.map((entry, i) => (
                                                                                                        <Typography variant="body2" key={entry.id}>
                                                                                                            {i + 1}. {entry.subjectName} - {entry.lessonName}
                                                                                                        </Typography>
                                                                                                    ))}
                                                                                                </Stack>
                                                                                            )}
                                                            ...
                                                            <UIStream
                                                                ...
                                                                builder={(viewSnap) => {
                                                                    ...
                                                                    return (
                                                                        <UIStream
                                                                            initialData={bloc.getField('draftLessons') ?? []}
                                                                            stream={bloc.getStream('draftLessons')}
                                                                            builder={(draftSnap) => {
                                                                                const draft = draftSnap.data ?? [];
                                                                                const lessonsOfPickedSubject: QuizLessonLite[] = pickSubjectId === ''
                                                                                    ? []
                                                                                    : (lessonsBySubject[pickSubjectId as number] ?? []);
                                                                                return (
                                                                                    <AppDialog
                                                                                                    ...
                                                                                                    <List dense>
                                                                                                        {draft.map((item: any, index: number) => (
                                                                                                            <ListItem
                                                                                                                ...
                                                                                                                secondaryAction={
                                                                                                                    <Stack direction="row">
                                                                                                                        <IconButton size="small" disabled={index === 0} onClick={() => bloc.moveDraftLesson(index, -1)}>
                                                                                                                            <ArrowUpwardOutlined fontSize="small" />
                                                                                                                        </IconButton>
                                                                                                                        <IconButton size="small" disabled={index === draft.length - 1} onClick={() => bloc.moveDraftLesson(index, 1)}>
                                                                                                                            <ArrowDownwardOutlined fontSize="small" />
                                                                                                                        </IconButton>
                                                                                                                        <IconButton size="small" color="error" onClick={() => bloc.removeDraftLesson(index)}>
                                                                                                                            <DeleteOutlined fontSize="small" />
                                                                                                                        </IconButton>
                                                                                                                    </Stack>
                                                                                                                }
                                                                                                            >
                                                                                                                <ListItemText primary={`${index + 1}. ${item.subjectName} - ${item.lessonName}`} />
                                                                                                            </ListItem>
                                                                                                        ))}
                                                                                                    </List>
                                                                                                )}
                                                                                                ...
                                                                                                <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap" useFlexGap>
                                                                                                    <FormControl size="small" sx={{ minWidth: 160 }}>
                                                                                                        ...
                                                                                                        <Select
                                                                                                            ...
                                                                                                            value={pickSubjectId}
                                                                                                            onChange={(e) => { setPickSubjectId(Number(e.target.value)); setPickLessonId(''); }}
                                                                                                        >
                                                                                                            {subjects.map((s) => <MenuItem key={s.id} value={s.id}>{s.name}</MenuItem>)}
                                                                                                        </Select>
                                                                                                    </FormControl>
                                                                                                    <FormControl size="small" sx={{ minWidth: 200 }} disabled={pickSubjectId === ''}>
                                                                                                        <InputLabel>{t('quiz-lessons')}</InputLabel>
                                                                                                        <Select
                                                                                                            label={t('quiz-lessons')}
                                                                                                            value={pickLessonId}
                                                                                                            onChange={(e) => setPickLessonId(Number(e.target.value))}
                                                                                                        >
                                                                                                            {lessonsOfPickedSubject.map((l) => <MenuItem key={l.id} value={l.id}>{l.name}</MenuItem>)}
                                                                                                        </Select>
                                                                                                    </FormControl>
                                                                                                    <Button
                                                                                                        variant="outlined"
                                                                                                        startIcon={<AddOutlined />}
                                                                                                        disabled={pickSubjectId === '' || pickLessonId === ''}
                                                                                                        onClick={() => addLesson(subjects, lessonsBySubject)}
                                                                                                    >
                                                                                                        {t('add')}
                                                                                                    </Button>
                                                                                                </Stack>
